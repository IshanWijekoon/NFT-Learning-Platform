...
$columns_to_check = [
    ...
    'video_path' => "ALTER TABLE courses ADD COLUMN video_path VARCHAR(500) DEFAULT NULL",
    'nft_certificate_image' => "ALTER TABLE courses ADD COLUMN nft_certificate_image VARCHAR(255) DEFAULT NULL"
];

foreach ($columns_to_check as $column => $alter_sql) {
    $check_col = "SHOW COLUMNS FROM courses LIKE '$column'";
    $col_result = mysqli_query($conn, $check_col);
    
    if (mysqli_num_rows($col_result) == 0) {
        if (!mysqli_query($conn, $alter_sql)) {
            echo json_encode(['success' => false, 'message' => "Failed to add $column column to database"]);
            exit();
        }
    }
}
...
$stmt = $conn->prepare("INSERT INTO courses (creator_id, $title_col, description, category, price, $duration_col, thumbnail, video_path, nft_certificate_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("isssdisss", $creator_id, $courseName, $description, $category, $price, $duration, $thumbnailPath, $videoPath, $certificatePath);
...
if ($stmt->execute()) {
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Course with NFT certificate created successfully',
        'course_id' => $course_id,
